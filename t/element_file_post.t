...
is( $headers->[0]{'Content-Length'}, 13 );
is( $headers->[0]{'Content-Type'},   'text/plain' );

my $header = $form->get_field('hello_world')->header;

is( $header->{'Content-Length'}, 13 );
is( $header->{'Content-Type'},   'text/plain' );
