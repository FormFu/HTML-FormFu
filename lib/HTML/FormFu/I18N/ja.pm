...
our %Lexicon = (
    ...
    form_constraint_ascii       => 'ASCII範囲外の不正な文字が使用されています',
    ...
    form_constraint_dependon    => "項目'[_1]' が入力されている場合は、この項目も必須です",
    form_constraint_email       => 'メールアドレスの書式が不正です',
    form_constraint_equal       => "'[_1]'と値が一致しません",
    form_constraint_file        => '値はファイルではありません',
    form_constraint_file_mime   => '不正な種別のファイルが指定されています',
    form_constraint_file_maxsize   => '使用できるファイルの最大容量は[_1] バイトです',
    form_constraint_file_minsize   => 'ファイルの最小容量は[_1] バイト以上です',
    form_constraint_file_size      => 'ファイル容量は[_1] から [_2] バイトの間である必要があります',
    form_constraint_integer     => '整数を指定してください',
    form_constraint_length      => '[_1]文字から[_2]文字の長さである必要があります',
    ...
    form_inflator_imager           => '画像データの処理に問題がありました',
    form_validator_imager_size     => '画像データの容量が大きすぎます',
    form_transformer_imager        => '画像データの処理に問題がありました',
);
