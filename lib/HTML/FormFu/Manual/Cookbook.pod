...
HTML::FormFu stores the markup for individual elements in templates files. 
You only need to install a local copy of the temlate files if you plan on 
customising them. If you're using the L<Catalyst> web framework, 
install L<Catalyst::Controller::HTML::FormFu> and run the following command:
...
HTML::FormFu will automatically find it's template files in your C<@INC> 
directories.

If you have a local copy of the template files, you can set the path with the 
following YAML config:

    tt_args:
      INCLUDE_PATH: "/path/to/templates"

If you're using L<Catalyst::Controller::HTML::FormFu> and don't have the 
templates in the default C<root/formfu> directory, you can set the directory 
for all your forms, application-wide, by setting this in your C<myapp.yml> 
...
In this case, C<tt_args> will be passed to the constructor of 
