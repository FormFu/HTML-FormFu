...
<input[% IF self.name.defined %] name="[% self.name %]"[% END %] type="[% self.field_type %]"[% IF self.value.defined %] value="[% self.value %]"[% END %][% process_attrs(self.attrs) %] />__CPAN_HTML_FormFu__multi_rtl__
[% INCLUDE $self.field_filename %][% IF self.label.defined %]
[% INCLUDE $self.label_filename %][% END %]__CPAN_HTML_FormFu__block__
[% INCLUDE start_block %]
...
[% WRAPPER field %]<button name="[% self.name %]" type="[% self.field_type %]"[% IF self.value.defined %] value="[% self.value %]"[% END %][% process_attrs(self.attrs) %]>[% self.content %]</button>[% END %]__CPAN_HTML_FormFu__form__
[% INCLUDE start_form %]
...
[% END %][% END %]</span>__CPAN_HTML_FormFu__textarea_tag__
...
[% IF self.tag.defined %]</[% self.tag %]>[% END %]__CPAN_HTML_FormFu__radiogroup__
...
[% IF self.container_tag.defined %]<[% self.container_tag %][% process_attrs(self.container_attributes) %]>[% END %][% IF self.label.defined && self.label_filename == 'legend' %]
